...
class data_generation(object):
    def __init__(self, type, logger):
        ...
        self.train_users = []
        self.train_items = []
        self.train_pre_items = []
        self.train_pre_items_length = []
        self.train_ratings = []
        self.train_labels = []
        ...
        for line in self.data.values:
            if is_first_line == 1:
                ...
                for i in range(len(items_id)):
                    itm = items_id[i].split(':')
                    self.train_users.append(int(user_id))
                    self.train_items.append(int(itm[0]))
                    if i >= self.pre_l:
                        pre_itm  = [int(j.split(':')[0]) for j in items_id[i-self.pre_l:i]]
                        pre_itm_l = [1]*self.pre_l
                    else:
                        pre_itm = [0]*(self.pre_l-i)+[int(j.split(':')[0]) for j in items_id[:i]]
                        pre_itm_l = [0]*(self.pre_l-i)+[1] * i

                    self.train_pre_items.append(pre_itm)
                    self.train_pre_items_length.append(pre_itm_l)
    ...
    def gen_test_data(self):
        ...
        for u in self.test_users:
            for item_id in self.test_candidate_items:
                ...
                if len(self.user_purchased_item[u]) >= self.pre_l:
                    pre_itm = list(self.user_purchased_item[u][-self.pre_l:])
                    pre_itm_l = [1] * self.pre_l
                else:
                    pre_itm = [0] * (self.pre_l - len(self.user_purchased_item[u])) + list(self.user_purchased_item[u])
                    pre_itm_l = [0] * (self.pre_l - len(self.user_purchased_item[u])) + [1] * len(self.user_purchased_item[u])

                self.test_pre_items.append(pre_itm)
                self.test_pre_items_length.append(pre_itm_l)
    ...
    def gen_train_batch_data(self, batch_size):
        ...
        if self.train_batch_id + batch_size >= l:
            ...
            batch_pre_items = self.train_pre_items[self.train_batch_id:] + self.train_pre_items[:self.train_batch_id + batch_size - l]
            batch_pre_items_len = self.train_pre_items_length[self.train_batch_id:] + self.train_pre_items_length[:self.train_batch_id + batch_size - l]
            ...
            batch_pre_items = self.train_pre_items[self.train_batch_id:self.train_batch_id + batch_size]
            batch_pre_items_len = self.train_pre_items_length[self.train_batch_id:self.train_batch_id + batch_size]
        ...
        return batch_users, batch_items, batch_pre_items, batch_pre_items_len, batch_ratings, batch_labels
    ...
    def gen_test_batch_data(self, user_number):
        ...
        test_batch_pre_items = self.test_pre_items[self.test_batch_id:self.test_batch_id + batch_size]
        test_batch_pre_items_len = self.test_pre_items_length[self.test_batch_id:self.test_batch_id + batch_size]
        self.test_batch_id = self.test_batch_id + batch_size

        return test_batch_users, test_batch_items, test_batch_pre_items,test_batch_pre_items_len
...
class rum():
    def __init__(self, data_type):
        ...
        self.batch_size = 1 # should be fixed as 1
        self.memory_rows = self.dg.pre_l
        ...
        self.item_pre_id = tf.placeholder(tf.int32, shape=[None, self.memory_rows], name='item_pre_id')
        self.item_pre_len = tf.placeholder(tf.int32, shape=[None, self.memory_rows], name='item_pre_len')
        ...
        self.memory_pointer = np.zeros(self.user_number)
        self.in_weight = tf.Variable(tf.zeros((self.global_dimension, 1), dtype='float32'), 'in_weight')
    ...
    def read_memory(self, item_embedding, item_pre_embedding, len):
        len = tf.to_float(len)
        self.weight = tf.nn.softmax(tf.multiply(tf.matmul(item_pre_embedding, tf.expand_dims(item_embedding, axis=2)),tf.expand_dims(len, axis=2)))
    ...
    def build_model(self):
        ...
        self.item_pre_embedding = tf.nn.embedding_lookup(self.item_embedding_matrix, self.item_pre_id)

        self.memory_out = self.read_memory(self.item_embedding, self.item_pre_embedding, self.item_pre_len)
        ...
        #self.test_score = tf.reduce_sum(tf.multiply(self.user_embedding, self.item_embedding), axis=1)
        ...
        with tf.Session() as self.sess:
            ...
            for iter in range(self.iteration):
                ...
                while self.step * self.batch_size <= self.dg.records_number:
                    user_batch, item_batch, item_pre_batch, item_pre_batch_len, rating_batch, label = self.dg.gen_train_batch_data(self.batch_size)


                    if label[0] == 1:
                        self.sess.run([self.intention_optimizer], feed_dict={self.user_id: user_batch, self.item_id: item_batch,
                                                                             self.item_pre_id: item_pre_batch, self.item_pre_len: item_pre_batch_len,
                        ...
                        self.sess.run([self.memory],
                                      feed_dict={self.user_id: user_batch, self.item_id: item_batch,
                                                 self.item_pre_id: item_pre_batch,
                                                 self.item_pre_len: item_pre_batch_len,
                                                 self.rating: rating_batch, self.label: label,
                                                 self.len: self.batch_size})
    ...
    def evaluate(self):
        ...
        for i in range(len(self.test_users) / user_number):
            batch_users, batch_items,test_pre_items,test_pre_items_len = self.dg.gen_test_batch_data(user_number)
            top_k_value, top_k_index = self.sess.run(
                ...
                feed_dict={self.user_id: batch_users,
                           ...
                           self.item_pre_id: test_pre_items,
                           self.item_pre_len: test_pre_items_len})
